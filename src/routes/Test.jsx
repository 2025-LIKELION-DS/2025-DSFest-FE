  ...
  return(
    <>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', backgroundColor: 'transparent' }}>
        ...
        <ButtonAdminSingle text={'부스 QR 및 비밀번호 생성'} color={`${palette.grayscale.white}`} onClick={buttonAdminSingleClick}/>
        <ButtonAdminSingle text={'부스 QR 및 비밀번호 목록'} color={`${palette.grayscale.white}`} onClick={buttonAdminSingleClick}/>
        ...
        <ModalAdmin text={'공지사항을 수정하시겠습니까?'} confirmText={'확인'} onClose={moadlAdminClose} onConfirm={moadlAdminConfirm}/>
        <ModalAdmin text={'공지사항을 업로드하시겠습니까?'} confirmText={'확인'} onClose={moadlAdminClose} onConfirm={moadlAdminConfirm}/>
        <ModalAdmin text={'공지사항을 삭제하시겠습니까?'} confirmText={'삭제'} onClose={moadlAdminClose} onConfirm={moadlAdminConfirm}/>

        <div>📌puzzle</div>
        <div>✔️Input_login</div>
        <InputLogin placeholder={'닉네임을 입력해주세요'} type={'text'} value={inputLoginID} onChange={inputLoginIDChange}/>
        <InputLogin placeholder={'비밀번호를 입력해주세요'} type={'password'} value={inputLoginPWD} onChange={inputLoginPWDChange}/>

        <div>✔️Button_modal_dual</div>
        <ButtonModalDual contentL={'부스 위치 확인'} onClickL={buttonModalDualLeftClick} contentR={'퍼즐 채우기'} onClickR={buttonModalDualRightClick}/>
        <ButtonModalDual contentL={'QR코드 재인증'} onClickL={buttonModalDualLeftClick} contentR={'비밀번호 입력'} onClickR={buttonModalDualRightClick}/>
        <ButtonModalDual contentL={'이전'} onClickL={buttonModalDualLeftClick} contentR={'수령 완료'} onClickR={buttonModalDualRightClick}/>
